        ...
        if isinstance(page, str):
            for data in DATABASE.values():
                if data['html'] == page:
                    ...
                    return render(request, "store/product.html", context={"product": data,"prod_category":data_category[:5]})
...
def coupon_check_view(request, name_coupon):
    ...
    if request.method == "GET":
        if name_coupon in DATA_COUPON:
            ...
            return JsonResponse({'is_valid':coupon['is_valid'], 'discount':coupon['value']})
